	...
	*/
	...
	MagnifierF.prototype = {
		init: function(id){
			...
			this.specBox = Get.byClass("proItems",this.magWrap)[0];
		...
		setEleFn: function(){   
			...
			_is.oMD.className = "zoomDrag"; 
			_is.oMD.style.cssText = 'width:' + _is.mW/2 +'px;height:' + _is.mH/2 + 'px;';
			_is.oMD.innerHTML = "&nbsp;";

			_is.oMP =  document.createElement('div');
			_is.oMP.className = 'MagnifierPop';
			_is.oMP.style.cssText =  'right:' + (-_is.mW-15) + 'px;';  
			
			_is.oMI = document.createElement('div');
			_is.oMI.className ='viewBigImg';
		...
		specFn: function(){
			var _is = this, _oSpImg, _oISrc,
				oLBtn = Get.byClass("prevBtn",_is.magWrap)[0],
				oRBtn = Get.byClass("nextBtn",_is.magWrap)[0],
			...
			function moveFn(event){
				...
				if (target.className.indexOf("nextBtn") > -1 ) {
					r = ++n; 
					if (r > _len - 5)
				...
				if (target.className.indexOf("prevBtn") > -1 ) {
					l = --n; 
					if (l < 1)
						Element.removeClass(target,"on");
